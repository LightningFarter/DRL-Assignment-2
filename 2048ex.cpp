 ...
 * networks for the game 2048," CIG 2014. [2] I-C. Wu, K.-H. Yeh, C.-C. Liang,
 * C.-C. Chang, and H. Chiang, "Multi-stage temporal difference learning for
 * 2048," TAAI 2014. [3] K. Matsuzaki, "Systematic selection of N-tuple networks
 * with consideration of interinfluence for game 2048," TAAI 2016.
 */
#include <algorithm>
#include <array>
#include <cmath>
...
#include <string>
#include <vector>
#include <chrono> // Include the time library
    ...
    public:
        pattern(const std::vector<int> &p, int iso = 8)
            : feature(1 << (p.size() * 4)) {
                 ...
                 * +------------------------+       +------------------------+
                 * the left side is an original board and the right side is its clockwise
                 * rotation
                 *
                ...
                for (size_t i = 0; i < (1ULL << (p.size() * 4)); i++) {
                    operator[](i) = 320000.0f;
                }
            }
...
class move {
    ...
    public:
        ...
        bool operator<(const move &s) const {
            return before == s.before && esti < s.esti;
        }
...
class learning {
    public:
         ...
         */
        template <typename feature_t> void add_feature(feature_t &&f) {
        ...
        float compute_best_move_value(const board &current_board) const {
            float best_value = -std::numeric_limits<float>::max();
            move possible_moves[4] = {move(current_board, 0), move(current_board, 1), 
                                      move(current_board, 2), move(current_board, 3)};
            
            for (move &next_mv : possible_moves) {
                if (next_mv.is_valid()) {
                    const board &afterstate = next_mv.afterstate();
                    int reward = next_mv.reward();
                    float expected_value = 0.0f;
    
                    // Collect all empty cells in the afterstate
                    ...
                    if (num_empty == 0) {
                        ...
                        // Calculate expected value from game's random tile placements
                        for (int cell : empty_cells) {
                            // Place tile 2 (90% chance)
                            board new_board_2 = afterstate;
                            new_board_2.set(cell, 2);
                            float value_2 = estimate(new_board_2);
    
                            // Place tile 4 (10% chance)
                            board new_board_4 = afterstate;
                            new_board_4.set(cell, 4);
                            float value_4 = estimate(new_board_4);
    
                            // Weight by probability and number of empty cells
                            expected_value += (0.9f / num_empty) * value_2 + 
                                              (0.1f / num_empty) * value_4;
                        }
                    }
    
                    // Total value for this move: reward + expected future value
                    float total_value = reward + expected_value;
            ...
            return best_value;  // Best value from all possible moves
        }
         ...
         *  note that the last record DOES NOT contain valid afterstate, action, and
         * reward
         */
        void learn_from_episode(std::vector<move> &path, float alpha = 0.1) {
            float target = 0;
            for (path.pop_back() /* ignore the last record */; path.size();
                    path.pop_back()) {
                move &mv = path.back();
         ...
         *
         * the statistic contains average, maximum scores, and tile distributions,
         * e.g.,
         *
         ...
         *         8192    14.8%   (14.8%)
         *
         * is the statistic from the 99001st to the 100000th games (assuming unit =
         * 1000), where '100000': current iteration, i.e., number of games trained
         *  'avg = 68663.7  max = 177508': the average score is 68663.7
         *                                 the maximum score is 177508
         *  '2048 91.2% (22.5%)': 91.2% of games reached 2048-tiles, i.e., win rate of
         * 2048-tile 22.5% of games terminated with 2048-tiles (the largest tile)
         */
         ...
         * the required features must be added, i.e., add_feature(...), before calling
         * this function
         */
...
int main(int argc, const char *argv[]) {
    ...
    // initialize the features of the 8x6-tuple network
    tdl.add_feature(pattern({0, 1, 2, 4, 5, 6}));
    tdl.add_feature(pattern({1, 2, 5, 6, 9, 13}));
    ...
    {
        debug << i << " ";
        tdl.get_feature(i)->print_weight();
    }
    ...
    for (size_t n = 1; n <= total; n++) {
        ...
        debug << "end episode " << std::dec << score << std::endl;
